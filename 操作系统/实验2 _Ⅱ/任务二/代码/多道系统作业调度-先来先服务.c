...
void inital() {
    ...
    for (i = 0; i < n; i++) {
        ...
        if (ready == NULL) {
            ...
            // 找到第一个到达时间大于当前作业的节点
            while (curr != NULL && curr->reachtime <= p->reachtime) {
                prev = curr;
                curr = curr->next;
            }
            if (prev == NULL) {
...
void disp(JCB *q) {
    printf("\n作业%s状态:%c\n", q->name, q->state);
    printf("到达时间:%d 内存:%dKB\n", q->reachtime, q->memory);
...
void show_status() {
    ...
    p = ready;
    while (p != NULL) {
        printf("%s(T%d)->", p->name, p->reachtime);
        p = p->next;
    }
    printf("NULL\n");
